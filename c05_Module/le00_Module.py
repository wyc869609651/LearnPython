...
# 举个栗子，一个abc.py的文件就是一个名字叫abc的模块，一个xyz.py的文件就是一个名字叫xyz的模块。
...
# 引入了包以后，只要顶层的包名不与别人冲突，那所有模块都不会与别人冲突。在线，abc.py模块的名字
# 就变成了mycompany.abc，类似的，xyz.py的模块名变成了mycompany.xyz。

# 请注意，每一个包目录下面都会有一个__init__.py的文件，这个文件是必须存在的，否则，Python就把
# 这个目录当成普通目录，而不是一个包。__init__.py可以是空文件，也可以有Python代码，因为__init__.py
...
# 文件www.py的模块名就是mycompany.web.www，两个文件utils.py的模块名分别是mycompany.utils和
# mycompany.web.utils。

# 注意：自己创建模块时要注意命名，不能喝Python自带的模块名称冲突。例如，系统自带了sys模块，自己的
# 模块就可命名为sys.py,否则将无法导入系统自带的sys模块。
...
# 2、模块名不要和系统模块名冲突，最好先查看系统是否已经存在该模块，检查方法是在Python交互环境执行
# import abc,如成功则说明系统存在此模块。
