...
# 比如说我们要定义一个Hello的class，就写一个hello.py模块：
# class Hello(object):
#     def hello(self, name='name'):
#         print('Hello, %s.' % name)
# 当Python解释器载入hello模块时，就会依次执行该模块的所有语句，执行结果就是动态创建一个Hello
# 的class对象，测试如下：
from c07_OOPAdvanced.hello import Hello
h = Hello()
h.hello()
print(type(Hello))
print(type(h))
...
# 通过type()函数创建的类和直接写class是完全一样的。因为Python解释器遇到class定义时，仅仅是
...
# 运行期创建类，必须狗仔源代码字符串在调用编译器，或者借助一些工具生成字节码实现，本质上
# 都是动态编译，会非常复杂。


# metaclass
# 除了时候用type()动态创建类以外，需要控制类的创建行为，还可以使用metaclass。
...
# 我们先看一个简单的例子，这个metaclass可以给我们自定义的MyList添加一个add方法：
...
# ListMetaclass.__new__()来创建，在此，我们可以修改类的定义，比如，加上新的方法，然后，
# 返回修改后的定义。
# __new__()方法接收到的参数依次是：
# 1.当前准备创建的类的对象；
# 2.类的名字；
...
# 要编写一个ORM框架，所有的类都只能动态定义，因为只有使用者这才能根据表的结构定义出对应的类来。
...
# 在Field的基础上，进一步定义各种类型Field，比如StringField， IntegerField等等：
...
#     1.排除掉对Model类的修改；
#     2.在当前类（比如User）中查找定义的类的所有属性，如果找到一个Field属性，就把它保存到
#       一个__mappings__的dict中，同时从类属性中删除该Field属性，否则，容易造成运行时错误
#       （实例的属性会遮盖类的同名属性）；
#     3.把表名保存到__table__中，这里简化为表名默认为类名。
# 在Model类中，就可以定义各种操作数据库的方法，比如save()，delete()，find()，update等等。
# 我们实现了save()方法，把一个实例保存到数据库中。因为有表名，属性到字段的映射和属性值的集合，
# 就可以构造出INSERT语句。
# 编写代码试试：
# u = User(id=12345, name='Michael', email='test@orm.org', password='my-pwd')
# u.save()