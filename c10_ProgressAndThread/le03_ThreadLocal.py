...
# 因为局部变量只有线程自己看得见，不会影响其他线程，而全局变量的修改必须加锁。
...
# 如果用一个全局dict存放所有的Student对象，然后以thread自身作为key获得线程对应的
# Student对象如何？
# global_dict = {}
#
# def std_thread(name):
...
# 但互不影响。你可以把local_student看成全局变量，但每个属性如local_school.student
# 都是线程的局部变量，可以任意读写而互不干扰，也不用管理锁的问题，ThreadLocal内部会处理。

# 可以理解为全局变量local_school是一个dict，不但可以用local_school.student，还可以绑定
