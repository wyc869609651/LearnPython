...
# 由于这个QueueManager只从网络上获取Queue，多以注册时只提供名字：
...
# 端口和验证码主要保持与task_master.py设置的完全一致：
...
#
# authkey有什么用？这是为了保证两台机器正常通信，不被其他机器恶意干扰。如果task_worker.py的
# authkey和task_master.py的authkey不一致，肯定连接不上。
...
# 比如发送一个处理日志文件的任务，就不要发送几百兆的日志文件本身，而是发送日志文件
