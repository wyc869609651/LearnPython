...
# start_response('200 OK', [('Content-Type', 'text/html')])
#
# 就发送了HTTP响应的Header，注意Header只能发送一次，也就是只能调用一次start_response()函数。start_response()
...
#
# 有了WSGI，我们关心的就是如何从environ这个dict对象拿到HTTP请求信息，然后构造HTML，通过start_response()
# 发送Header，最后返回Body。
#
# 整个application()函数本身没有涉及到任何解析HTTP的部分，也就是说，底层代码不需要我们自己编写，我们只负责在
# 更高层次上考虑如何响应请求就可以了。
#
...
# 然后，再编写一个server.py负责启动WSGI服务器，加载application()函数：
# server.py
# 从wsgiref模块导入：
